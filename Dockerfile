...
# 1 Build docker image locally with Dockerfile: docker build --pull --rm -f "Dockerfile" -t gcr.io/projectfj/scheduledtask:1.0.1 "."
# 2 check if image got created: docker images
# 3 Push to Google-Container-Registry (GCR): docker push gcr.io/projectfj/scheduledtask:1.0.1
